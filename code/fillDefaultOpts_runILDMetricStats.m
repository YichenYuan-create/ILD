function opts = fillDefaultOpts_runILDMetricStats(opts)

    if ~isfield(opts, 'metricName') || isempty(opts.metricName)
        opts.metricName = 'metric';
    end
    ...
    if ~isfield(opts, 'signrankCenter') || isempty(opts.signrankCenter)
        opts.signrankCenter = 0;
    end
    ...
    if ~isfield(opts, 'appendPairwiseToMultcomp') || isempty(opts.appendPairwiseToMultcomp)
        opts.appendPairwiseToMultcomp = true;
    end
end